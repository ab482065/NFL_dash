...
app = dash.Dash(__name__, prevent_initial_callbacks="initial_duplicate",  external_stylesheets=[
...
def create_tab_content(label, position, stat_options):
    return dbc.Tab(label=label, children=[
        dbc.Row([
            dbc.Col([
                html.H5(f"Choose a {position.lower()} to view their statistics from 2019-2022.",
                        style={'margin-top': '20px', 'font-weight': 'bold'}),
                dcc.Dropdown(
                    id=f'{position.lower()}-dropdown',
                    options=[{'label': player, 'value': player}
                             for player in df[df['position'] == position]['player'].unique()],
                    # Default selected player
                    value=df[df['position'] == position]['player'].iloc[0],
                    multi=False,
                ),
                dcc.RadioItems(
                    id=f'{position.lower()}-stat-radio',
                    options=stat_options,
                    # Default selected radio item
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(id='detailed-stats', width=6),
        dbc.Col(id='detailed-viz-container', width=6),
    ], className="mt-4"),
    # dbc.Row([
...
@app.callback(
    ...
    Output('detailed-viz-container', 'children'),
...
def update_qb_stats(selected_qb, selected_stat, click_data):
    ...
    qb_data = df[df['player'] == selected_qb]

    # if selected_qb:
    #     qb_data = get_player_subset_of_dataset(
    #         selected_qb, ["game_id", "player", selected_stat])
    ...
    if click_data is not None:
        player = click_data['points'][0]['customdata'][0]
        ...
        if not game_stats.empty:
            detailed_stats = html.Div([
                html.H3(
                    f"Detailed Statistics for Game ID: {selected_game_id}"),
                html.Table([
                    html.Tr([html.Th("Pass Yards"), html.Td(
                        game_stats['pass_yds'].values[0])]),
                    html.Tr([html.Th("Pass Attempts"), html.Td(
                        game_stats['pass_att'].values[0])]),
                    html.Tr([html.Th("Pass Completions"), html.Td(
                        game_stats['pass_cmp'].values[0])]),
                    html.Tr([html.Th("Interceptions"), html.Td(
                        game_stats['pass_int'].values[0])]),
                    html.Tr([html.Th("Touchdowns"), html.Td(
                        game_stats['pass_td'].values[0])]),
                ]),
            ])

        player_df = qb_data[qb_data['player'] == player]

        scatter_graph = px.scatter(player_df, x='game_date', y=[
            'pass_yds', 'pass_att', 'pass_cmp', 'pass_int', 'pass_td'], log_y=True, size='pass_yds', size_max=15,
            color_discrete_sequence=px.colors.qualitative.Bold, opacity=1)
        scatter_graph.update_layout(plot_bgcolor='rgba(0,0,0,0)')

        detailed_viz_container = dbc.Container([
            html.H5(f"Visualization Options for {selected_qb}", style={
                    'font-weight': 'bold'}),
            dcc.Graph(id='qb-detailed-viz', figure=scatter_graph),
        ])
...
@app.callback(
    ...
    Output('detailed-stats', 'children', allow_duplicate=True),
    Output('detailed-viz-container', 'children', allow_duplicate=True),
...
def update_rb_stats(selected_rb, selected_stat, click_data):
    ...
    rb_data = df[df['player'] == selected_rb]

    # if selected_rb:
    #     rb_data = get_player_subset_of_dataset(
    #         selected_rb, ["game_id", "player", selected_stat])
    ...
    try:
        fig = px.bar(rb_data, x='game_id', y=selected_stat,
                     custom_data=['player'], title=title)
        ...
        if not game_stats.empty:
            detailed_stats = html.Div([
                html.H3(
                    f"Detailed Statistics for Game ID: {selected_game_id}"),
                html.Table([
                    html.Tr([html.Th("Rush Yards"), html.Td(
                        game_stats['rush_yds'].values[0])]),
                    html.Tr([html.Th("Rush Attempts"), html.Td(
                        game_stats['rush_att'].values[0])]),
                    html.Tr([html.Th("Rushing Touchdowns"), html.Td(
                        game_stats['rush_td'].values[0])]),
                    html.Tr([html.Th("Longest Rush"), html.Td(
                        game_stats['rush_long'].values[0])])
                ]),
            ])
        player_df = rb_data[rb_data['player'] == player]

        scatter_graph_two = px.scatter(player_df, x='game_date', y=[
            'rush_yds', 'rush_att', 'rush_long', 'rush_td'], log_y=True, size='rush_yds', size_max=15,
            color_discrete_sequence=px.colors.qualitative.Bold, opacity=1)
        scatter_graph_two.update_layout(plot_bgcolor='rgba(0,0,0,0)')

        detailed_viz_container = dbc.Container([
            html.H5(f"Visualization Options for {selected_rb}", style={
                    'font-weight': 'bold'}),
            dcc.Graph(id='rb-detailed-viz',
                      figure=scatter_graph_two),
        ])
        # detailed_viz_container = dbc.Container([
...
def update_wr_stats(selected_wr, selected_stat, click_data):
    ...
    try:
        fig = px.bar(wr_data, x='game_id', y=selected_stat,
                     custom_data=['player'], title=title)
        ...
        if not game_stats.empty:
            detailed_stats = html.Div([
                html.H3(
                    f"Detailed Statistics for Game ID: {selected_game_id}"),
                html.Table([
                    html.Tr([html.Th("Receiving Yards"), html.Td(
                        game_stats['rec_yds'].values[0])]),
                    html.Tr([html.Th("Targets"), html.Td(
                        game_stats['targets'].values[0])]),
                    html.Tr([html.Th("Receiving Touchdowns"),
                            html.Td(game_stats['rec_td'].values[0])]),
                    html.Tr([html.Th("Longest Reception"), html.Td(
                        game_stats['rec_long'].values[0])])
                ]),
            ])
        player_df = wr_data[wr_data['player'] == player]

        scatter_graph_three = px.scatter(player_df, x='game_date', y=[
            'rec_yds', 'targets', 'rec_long', 'rec_td', 'rec'], log_y=True, size='rec_yds', size_max=15,
            color_discrete_sequence=px.colors.qualitative.Bold, opacity=1)
        scatter_graph_three.update_layout(plot_bgcolor='rgba(0,0,0,0)')

        detailed_viz_container = dbc.Container([
            html.H5(f"Visualization Options for {selected_wr}", style={
                    'font-weight': 'bold'}),
            dcc.Graph(id='rb-detailed-viz', figure=scatter_graph_three),
        ])
        detailed_viz_container = dbc.Container([
            html.H3(f"Visualization Options for {selected_wr}"),
            dcc.Graph(id='wr-detailed-viz',
                      figure=scatter_graph_three),
        ])
    return [fig, detailed_stats, detailed_viz_container]
